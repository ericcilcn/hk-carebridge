...
- Drug titles use `中文通用名（English generic name）`. Drug fact grids contain exactly six cells in this order: `通用名 / 商品名 / 药厂 / 药物类型 / 剂型与规格 / 常见应用`. Write the generic name, brand and manufacturer as the Chinese name followed by the English name in full-width parentheses.
- In drug titles, keep the Chinese and English generic names on one line wherever the viewport permits. Render the English name as a smaller secondary span rather than giving both languages the same visual weight.
...
- Use the standalone HK bridge mark from the approved transparent horizontal logo for browser favicons and app icons. Center it on the site's warm-white `#f5f1e8` square with safe padding; do not use the wordmark or the checkerboard-backed source image at small sizes.
- Use a 3:2 horizontal editorial image for the homepage `微信咨询` module. The scene should show a messaging contact point and organized written medical information without readable chat content, QR codes, logos, private data, medicine packages or purchase cues. Keep the four consultation steps in a compact two-by-two grid on wide screens and stack them on narrower screens.

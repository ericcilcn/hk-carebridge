...
- Use `微信咨询` as the primary CTA, and refer to the channel only as `微信`.
- The current service only accepts a written overview and returns a condition-summary framework, a tailored preparation checklist, and a consultation-question list.
...
- Cancer article titles are fixed as `肺癌`, `乳腺癌` and `结直肠癌`. Do not change their summaries, introductions, sections, lists or questions unless the user explicitly requests copy edits.
- Do not imply diagnosis, treatment advice, report interpretation, physician or institution recommendation, appointment booking, referral, medication sales, fees, or a guaranteed response time.
